...
def calc_graft(
    path: str, free_steps: int = 100000, calc_steps: int = 1e6, trigger: int = 1000, label: str=''
) -> None:
    ...
    sim.create_state_from_gsd(filename=os.path.join(path, f"init{label}.gsd"))
    ...
    gsd = hoomd.write.GSD(trigger=trigger, filename=os.path.join(path, f"output{label}.gsd"))
    ...
    np.savetxt(os.path.join(path, f"tension{label}.txt"), tension)
