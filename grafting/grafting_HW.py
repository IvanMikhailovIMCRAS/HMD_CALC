...
def grafting(num_grafting: int, n: int, g: int, path_to_files: str) -> None:
    track = ReadTrack(path=path_to_files)
    ...
    for i in list_top:
        ...
        if first == True:
            ...
            if len(dendron.angles) > 0:
                dendron_angles = [i, len(x) + 1, len(x) + 2]
                if len(dendron.angles) > 1:
                    dendron_angles = np.vstack(
                        [dendron_angles, np.asarray(dendron.angles[1:]) + len(x)]
                    )
            first = False
        else:
    ...
    track.btype[num_lipid * 11 : num_lipid * 11 + delta_n] = 4

    all_bonds = np.vstack([read_bonds(path=path_to_files), dendron_bonds])
    with open("BONDS_exp", "w") as f:
        f.writelines(
            f" num_bonds{len(all_bonds):13d}  num_atoms{len(x):13d}  BOX{track.box.x:14.7f}{track.box.y:17.7f}{track.box.z:17.7f}\n"
        )
    ...
    if len(dendron.angles) > 0:
        ...
        try:
            ...
            loging.write("File ANGLS was not read")
            loging.close()
    ...
    with open("COORD_exp", "w") as f:
        f.writelines(
            f"num_atoms{len(x):7d} box_size{track.box.x:15.10f}{track.box.y:17.10f}{track.box.z:17.10f}\n"
        )
        n = 0
